...
void INSSolver::ComputeAdvection(
  ...
  BL_PROFILE("INSSolver::ComputeAdvection()");
    ...
    // --- x-face advection ---
    {
      ...
      amrex::ParallelFor(
          bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
            ...
            au(i, j, k) = -(u(i, j, k) * dudx + v_at_u * dudy + w_at_u * dudz);
#else
    ...
    // --- y-face advection ---
    {
      ...
      amrex::ParallelFor(
          bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
            ...
            av(i, j, k) = -(u_at_v * dvdx + v(i, j, k) * dvdy + w_at_v * dvdz);
#else
    ...
    // --- z-face advection ---
    {
      ...
      amrex::ParallelFor(
          bx, [=] AMREX_GPU_DEVICE(int i, int j, int k) noexcept {
            ...
            aw(i, j, k) = -(u_at_w * dwdx + v_at_w * dwdy + w(i, j, k) * dwdz);
          });
    }
#endif
  }
